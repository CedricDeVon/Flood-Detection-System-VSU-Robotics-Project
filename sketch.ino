...
#define HCSR04_ULTRASONIC_DISTANCE_SENSOR_OUTPUT_PIN 5

#define MAXIMUM_DISTANCE_IN_INCHES 100
#define DISTANCE_MEASUREMENT 1 // Inches
#define SAFE_LEVEL_TOLERANCE_IN_INCHES 4
...
void displayDistanceToLCD(unsigned int distance)
{
    ...
    if (
        (currentTimeSinceArduinoInitializationInMilliseconds - timestampInMilliseconds) >=
        LIQUID_CRYSTAL_DISPLAY_DELAY_IN_MILLISECONDS)
    {
        timestampInMilliseconds = currentTimeSinceArduinoInitializationInMilliseconds;        
        ...
        byte currentFloodLevel =
            (distance >= SAFE_LEVEL_TOLERANCE_IN_INCHES) ? DANGER_LEVEL : SAFE_LEVEL;
        liquidCrystalDisplay.displayTextAtCoordinates(
...
void runtime()
{
    unsigned int distance = hcsr04UltrasonicDistanceCalculator.getIntegerConversion(hcsr04UltrasonicDistanceSensor.getAnalogInput());
